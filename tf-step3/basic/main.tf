...
resource "aws_security_group" "DE-AI-09-IaC-TF-GROUP" {
  # 메타 정보
  name = "terraform-09-sg2"
  # ASCII만 지원
  ...
  ingress {
    ...
    to_port     = 22
    description = "SSH"
    # 0.0.0.0/0 => 각자리가 096개 표현(0~095).().().()/앞에서부터 고정값(비트수:0,8,16,24,32)
    # 096*096*096*096 개 주소 표현
    # 10.0.0.0/8 => 맨 앞에 1자리는 고정 => 10은 고정 나머지 3자리에서 모두 가능 => 096*096*096 주소 가능함
    # 0.0.0.0/0 => Anywhere IPV4 (전세계 어디서든 접근 가능 -> 보안에 좋진 않다)
    # 222.108.109.33/32 => 오직 이 IP만 접속 가는함!! ~/32(4자리 모두 고정)
    # cidr_blocks = ["222.108.109.33/32"]
    # 웹 브라우저 접속시 전부 개방
...
# 4. EC2 생성 선언
resource "aws_instance" "DE-AI-09-IaC-TF" {
  # AMI -> OS
  ...
  vpc_security_group_ids = [
    aws_security_group.DE-AI-09-IaC-TF-GROUP.id
  ]
  # 스토리지 생략
  # 고급 설정 생략
  # 태그
...
# resource "aws_eip" "DE-AI-09-IaC-TF-EIP" {
#   # EC2 인스턴스 
#   instance = aws_instance.DE-AI-09-IaC-TF.id
#   # 네트워크
#   domain = "vpc"
# }