...
resource "aws_eks_cluster" "main" {
    ...
    # EKS Control plane이 AWS 리소스 관리할때 IAM Role
    ...
    compute_config {
      ...
      # 노드 구성 => 노드풀에서 가져와서 구성
      node_pools = [
        "gerneral-purpose", # web, was 등 pod 실행하는 용도의 node
        "system"            # 중요 시스템 pod용
      ]
    ...
    kubernetes_network_config {
      ...
      # 서비스에 해당되는 파트는 기존 vpc, pod, node의 사용하는 CIDR과 겹치면 x
    ...
    vpc_config {
      ...
      # 로컬 PC에서 public eks api 엔드포인트 접근 허용
      endpoint_public_access = true
      # cidr 제약(현재, 어떤 대역이던 접근 OK 구성)
      # public eks API Endpoint (CLI로 접근하여 명령어 전송 등 처리 모든 대역 OK)
    ...
    enabled_cluster_log_types = [
        ...
        "controllerManger", # Deployment, ReplicaSet 제어 > pod 수치/증감
...
resource "aws_eks_addon" "metrics_server" {
  # 매트릭 서버를 애드온할 대상 EKS 클러스터
  cluster_name = aws_eks_cluster.main.name
  # Add-on 이름
  addon_name = "metrics-server"
  # 이미 설정된 경우 -> 설정 덮어쓰기로 구성
