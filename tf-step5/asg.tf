...
# CPU 측정(트래픽 나름 비례) -> 50% 기준 설정(사내별 상이함) 
...
resource "aws_autoscaling_group" "web" {
  ...
  # EC2 를 lunch_template 이용하여 구성하겠다 -> 상세 구성 내용- 런치 템팔림
  launch_template {
    # 커스텀으로 구성한 런치 템플릿 참조
  ...
  # 태그 - 블록반복
  dynamic "tag" {
    # merge( 합병 - 기존태그(3개) + 신규 태그(2개) 합병하여 동적 EC2에 동적 세팅 샘플
...
resource "aws_autoscaling_group" "was" {
  ...
  health_check_type         = "ELB"
  # 240초 동안 헬스 체크 오류는 무시...ㅈㅁㄴrk rntjdtkd ej tlrksdl thjdyehla
...
resource "aws_autoscaling_policy" "web_cpu" {
  ...
  target_tracking_configuration {
    ...
    # 타겟값 50 => 평균 CPU 사용량 50% 높음 -> EC2 증가(감소) 자동 스케일링 진행
    # 2 ~ 4 사이에 구성
    target_value = 50
  }
}
