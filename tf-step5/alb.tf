...
resource "aws_lb_target_group" "web" {
  ...
  health_check {
    ...
    path                = "/health" # 체크하는 URL 값(경로)
    protocol            = "HTTP"    # 통신 방식
    matcher             = "200"     # 정상 응답 코드 값
    interval            = 30        # 헬스 체크 간격 (초)
    timeout             = 5         # 응답 대기 시간 (초)
    healthy_threshold   = 2         # 정상 상태 최소값 (2번 연속)
    unhealthy_threshold = 2         # 비정상 상태 실패값 (2연속)
  }
...
###################################################
# Internal ALB : WEB ASG <-> Internal ALB <-> WEB ASG
###################################################
