...
variable "availability_zones" {
  ...
  type        = list(string)
  default     = ["us-west-1a", "us-west-1c"] # 리전에 맞게 구성
  # 유효성 검사 표기 (가용영역이 2개 이하 or c 존이 없는 경우)
  validation {
...
variable "cluster_endpoint_public_access_cidrs" {
  description = "EKS Public API 접근 CIDR. 용도에 따라 제한할 수 있음, 실제는 개발자 IP 한정"
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
