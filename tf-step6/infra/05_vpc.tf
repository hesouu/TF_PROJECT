...
resource "aws_internet_gateway" "main" {
  ...
  tags = {
    Name = "${local.cluster_name}-iiggww"
  }
}
...
resource "aws_subnet" "public" {
  ...
  tags = {
    ...
    # 쿠버네티스 관련
    # 쿠버네티스가 외부용 로드밸런서 서브넷으로 식별할수 있도록 태그 구성
...
# 쿠버네티스가 해당 서브넷을 `내부 적용 ALB`등을 만들때 해당 서브넷 사용하도록 하는 태그 표식 추가
...
resource "aws_eip" "nat" {
  ...
  for_each = aws_subnet.public # a존, c존에 각각 IP 할당
